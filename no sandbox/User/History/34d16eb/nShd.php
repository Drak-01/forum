...
public function show(Group $group)
{
    ...
    // Charge le nombre de réponses en une seule requête
    $reponsesCounts = DB::table('reponses')
        ->select('question_id', DB::raw('count(*) as count'))
        ->whereIn('question_id', $group->questions->pluck('id'))
        ->groupBy('question_id')
        ->pluck('count', 'question_id');

    // Assignez les counts aux questions
    $group->questions->each(function ($question) use ($reponsesCounts) {
        $question->reponses_count = $reponsesCounts[$question->id] ?? 0;
    });
